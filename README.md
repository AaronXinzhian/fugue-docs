...
3. **机器相全程自动化,语义相才需要智能**:初始化时脚手架静态生成骨架(语义留 `TODO`);维护期 `geb_sync` 把 `[INPUT]` 行与清单表当作**视图**从代码重新生成——衍生数据不靠手抄、不搞对账,从根上消灭这一半的漂移。机器绝不假装理解语义。
4. **度的把握,防止协议自身制造熵**:生成文件、配置、vendored 依赖不加头;极小项目豁免 L2;文档的成本必须低于它消除的歧义。**递归分形**:子目录含 `PROJECT_INDEX.md` 即子项目(它的 L1 就是父级视角的 L2),检查与同步自动递归进入——monorepo 原生支持。
...
| 命令 | 作用 |
|------|------|
| `python3 scripts/geb_sync.py <项目目录>` | 机器字段同步:`[INPUT]` 行与清单表从代码重新生成(语义列保留),`--graph` 重绘依赖图 |
...
**验证程度,如实陈述**:小规模脚本判定对照实验(每场景 n=1,可复跑)+ 本仓库自举使用(每次提交 CI 以 --strict 自检)+ 多轮外部模型评审(全部意见与逐条处理记录见提交历史与归档);**尚无团队生产环境的长期数据**。理解测验与真实仓库案例研究在持续补充中。发现问题请开 issue——这个项目的迭代方式就是消化批评。
