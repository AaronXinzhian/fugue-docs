...
4. **過剰文書化の抑制**:生成ファイル・設定・vendored 依存にはヘッダを付けない。極小プロジェクトは L2 を免除。ドキュメントのコストは、それが解消する曖昧さを下回らなければなりません。**再帰フラクタル**:`PROJECT_INDEX.md` を持つサブディレクトリはサブプロジェクト(その L1 が親から見た L2 を兼ねる)。検査と同期は自動で再帰——monorepo をネイティブにサポート。
