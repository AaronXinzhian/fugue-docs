#!/usr/bin/env python3
"""
[INPUT]: 依赖 argparse, os, re, sys, geb_check, geb_scaffold
...
from geb_check import L1_NAMES, L2_NAMES, find_index_file, walk_project  # noqa: E402
...
def rebuild_table(index_path, files, analyses, dry=False):
    ...
    head_i = next((i for i, l in enumerate(lines)
                   if l.strip().rstrip(nl) in TABLE_HEADINGS or
                   l.strip() in TABLE_HEADINGS), None)
    if head_i is None:
        return None
...
def sync(root, graph=False, dry_run=False, prefix=""):
    subprojects = []
...
def main():
    ...
    if not os.path.isdir(root):
        ...
        return 2
    changed = sync(root, graph=args.graph, dry_run=args.dry_run)
    for line in changed:
        print(line)
