...
| geb_sync.py | 视图同步器:重写 L3 [INPUT] 行与 L2/L1 清单表(语义列保留);--graph 重绘依赖图;--changed git 增量并覆盖删除/重命名目录;小项目整表模式;递归进子项目 | _TODO, INPUT_TAG, read_keepnl(), write_keepnl(), sync_l3_input(), TABLE_HEADINGS, _ROW, rebuild_table(), rebuild_graph(), git_changed() |
