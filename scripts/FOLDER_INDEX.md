...
| geb_check.py | 同构性检查器:结构/--strict 漂移/--complete 占位三层;--if-adopted 采纳判定唯一事实源;--report 回环行;--emit-facts 机器事实源;小项目 profile 与子项目递归 | CODE_EXTENSIONS, EXCLUDED_DIRS, EXCLUDED_FILE_PATTERNS, L1_NAMES, L2_NAMES, L3_TAGS, L3_SCAN_LINES, SMALL_PROJECT_FILE_LIMIT, SMALL_PROJECT_DIR_LIMIT, is_small_project() |
...
| geb_sync.py | 视图同步器:重写 L3 [INPUT] 行与 L2/L1 清单表(语义列保留);--graph 重绘依赖图;--changed git 增量;小项目整表模式;递归进子项目 | _TODO, INPUT_TAG, read_keepnl(), write_keepnl(), sync_l3_input(), TABLE_HEADINGS, _ROW, rebuild_table(), rebuild_graph(), git_changed() |
