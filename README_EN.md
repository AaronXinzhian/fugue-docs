...
4. **Proportionality, so the protocol doesn't create entropy itself**: no headers for generated files / configs / vendored deps; tiny projects may skip L2. Documentation must cost less than the ambiguity it removes. **Recursive fractal**: a subdirectory with its own `PROJECT_INDEX.md` is a subproject (its L1 doubles as the parent's L2); checking and syncing recurse automatically — native monorepo support.
